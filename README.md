...
# Features
<a name="#features" />
This starter kit aims to get any project off the ground in as little time as possible. It mimics a premed website, so you can make modifications where needed to get a fully functioning, mostly optimized website in hours. It comes with a fully responsive design by default, and has a few bells and whistles to make for a nicer dev experience.

Some of those additional features include:
- **Eleventy** - Eleventy gives you access template languages within your project. This allows you to supercharge your HTML to allow for reusable components and layouts to keep your code as DRY as possible. The template language used in this kit is Nunjucks, but eleventy allows you to use multiple at the same time. It also comes with a range of plugins too...
- **Eleventy Images**. With this plugin, you no longer need to spend hours optimizing and reformatting your images. Just use the image shortCode, and eleventy will optimize, compress and convert your image to next-gen formats. Provided you write good code, you can get straight 90+ scores on Lighthouse out-the-box
- **Eleventy Navigation** - This simplifies the way you do navigation to just 3 lines within the formatter of your HTML. All the rest has been done for you.
- **Centralized Data** - To make it quicker and easier to get started and scale your project, there is a folder containing key information about the client. Should you then later want to change a phone number, address or name, it's all contained in one place.
- **Netlify CMS** - This starter kit comes with a blog, powered by Netlify CMS. All the config is done for you.

If any of the above doesn't make sense, don't worry! It is all explained in the [Get Started]() section. But first, it's best to get acquitted with the structure of the project...
...
### Source Folders
You'll be working in the src folder with this project. In here is an organized system with everything you'll need to make an awesome website.
...
- **_includes and _layouts** - These two folders allow for some of the templatefeatures within eleventy. Full page layout templates are stored in _layouts, and can be used in the 'layout' field within the formatter of your files. _includes contain the smaller, reusable components, such as the header or footer
...
- **assets** - For all non-HTML/CSS related assets, such as SVGs, JavaScript and images you don't want to be optimized.
- **blog** - All blog page markdown files are stored in here. Formatter for the blog post pages are stored in blog.json.
...
You may be wondering why there are two places to store images. This is to do with the eleventy image plugin. Images stored in src/images will be optimized and saved to the /public build folder, which includes a name change. assets/images contains images you don't want to be optimized by eleventy.

Because of how the build process works, and how the names of the image files change when being optimized, images from the CMS cannot be optimized with eleventy. On top of that, some images, like the logo, don't need to be resized or optimized, and thus can live here.
...
0. If this is your first time using this kit, I highly recommend you read [this article](https://www.hyunbinseo.medium.com/nunjucks-settings-for-vs-code-a0da0dc66b95) on setting up VSCode to use Nunjucks, the template language for this kit. This will give you access to autoformatting, syntax highlighting and emmet. 
...
If there are any sets of related or repeated data during the development of the website, consider making a new file in the _data folder, to keep data centralized. This keeps your code easy to maintain. A good example would be a menu - you could have menu data in a menu.json folder, then use a [nunjucks for loop](https://mozilla.github.io/nunjucks/templating.html#for) to iterate over the menu items. Then if the client requests a price update, you know exactly where to look.

### Layouts and Components
As mentioned in the File Structure section, there are folders to keep your layouts and components in order. All pages pull from the base.html file in _layouts. This contains the head, header, footer and a main tag where the page content is inserted. The header and footer themselves are components found in _includes.

Most of the <head> will be completed for you by filling out the formatter of each page (next section), but some things, like favicons and font preloads, will need to be manually adjusted.

The title tag is also preConfigured, giving an SEO optimized title for each page. Home page and interior page titles are treated differently, but this can all be changed in base.html
...
With the eleventy navigation plugin, you have access to an easy, scalable way to design a navigation. In the formatter, you can the data under eleventyNavigation is what controls the nav. The key is what name will be rendered on the navigation, while the order determines, well, what order the pages will appear, from smallest to largest.

If you don't want a page to be linked on the navigation, just leave the eleventyNavigation part of the formatter out.
...
There may be a case where you need to use an image or image source without using the shortCode. For example, as a CSS background-image or in the preload section of the formatter. For this, it's important to know what happens when you call a shortCode.
...
Move images you would like to be optimized into the /images folder. The other assets belong in /assets, including unoptimized images. Then, you will have to go through and replace all images with the {% image ... %} shortCode. This is still quicker than manually optimizing the images, however!
...
tagTitle: 'About'
preloadImg: '/images/cabinets2-1920w.web'
...
Normally, we set our permalink to be the page slug (pearled-piano-lessons/) but if they have a unique file path all you have to do is include it in the permalink. No slashes before the slug name, only 1 slash after as seen in the example above.
...
### CMS Modification
In the /admin folder there is a config.yml file, controlling the CMS. Most of it is already set up for you, assuming you just want a blog. You can read more about the [capabilities of the CMS here](https://www.netlifycms.org/docs/intro/)
